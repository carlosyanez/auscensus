...
```{r}
Metadata <-
  read_excel("data-raw/data-packs/Metadata/Metadata_2011_BCP_DataPack_R1.xlsx",
             sheet = 2,
             skip = 10) %>%
  as.data.table
```
...
```{r Region_key}
Region_key <- 
  fread(paste0('./data-raw/data-packs/data/', Region, '/AUST/2011Census_G01_AUST_', Region, '.csv')) %>%
  names %>%
  extract2(1)
```
...
freadG <- function(g, ...) {
  ...
  file_list <-
    list.files(path = file.path('./data-raw/data-packs/data/', Region),
               ...
               pattern = paste0('2011Census_G', g0, '[A-Z]?_AUST_', Region, '\\.csv$'),
               recursive = TRUE,
               full.names = TRUE)
...
```{r total_adults}
total_adults <- 
  fread("./data-raw/data-packs/data/AUST/2011Census_G01_AUS_AUS.csv") %>%
    ...
    decoder <- 
      switch(Region,
             "SA2" = SA21_decoder[ASGS_Structure == Region,
                                  .(SA2_MAIN21 = Census_Code_2011,
                                    SA2_NAME21 = Census_Name_2011)],
             "SA3" = SA21_decoder[ASGS_Structure == Region,
                                  .(SA3_MAIN21 = Census_Code_2011,
                                    SA3_NAME21 = Census_Name_2011)],
             "SA4" = SA21_decoder[ASGS_Structure == Region,
                                  .(SA4_MAIN21 = Census_Code_2011,
                                    SA4_NAME21 = Census_Name_2011)],
             "STE" = SA21_decoder[ASGS_Structure == Region,
                                  .(STE_MAIN21 = Census_Code_2011,
                                    STE_NAME21 = Census_Name_2011)],
             "LGA" = {
               NonABS_decoder[ASGS_Structure == Region,
                              .(Census_Code_2011, LGA_NAME21 = Census_Name_2011)]
             },
             "CED" = {
               NonABS_decoder[ASGS_Structure == Region,
                              .(Census_Code_2011, CED_NAME21 = Census_Name_2011)]
             },
             "SED" = {
               NonABS_decoder[ASGS_Structure == Region,
                              .(Census_Code_2011, SED_NAME21 = Census_Name_2011)]
             },
             "SSC" = {
               NonABS_decoder[ASGS_Structure == Region,
                              .(Census_Code_2011, SSC_NAME21 = Census_Name_2011)]
             },
             "GCCSA" = {
               read_excel("./data-raw/data-packs/Metadata/2011Census_geog_desc_1st_release.xlsx",
                          sheet = "2011_ASGS_GCCSA_Structure") %>%
                 as.data.table %>%
                 .[, .(Census_Code_2011, GCCSA_NAME21 = Census_Name_2011)]
             },
             NULL) %>%
      unique %>%
...
NonABS_decoder <-
  read_excel("./data-raw/data-packs/Metadata/2011Census_geog_desc_1st_release.xlsx",
             sheet = "2011_ASGS_Non_ABS_Structures") %>%
  as.data.table
```
...
CED_decoder <-
  read_excel("./data-raw/data-packs/Metadata/2011Census_geog_desc_1st_release.xlsx", 
             sheet = "2011_ASGS_Non-ABS_Structures") %>%
  setDT %>%
  .[ASGS_Structure == "CED"] %>%
  setnames("Census_Code_2011", Region_key) %>%
  setnames("Census_Name_2011", "Electoral_division") %>%
...
```{r Region__Age}
fread(paste0('./data-raw/data-packs/data/', Region, '/AUST/2011Census_G01_AUS_', Region, '.csv')) %>%
...
fread(paste0('./data-raw/data-packs/data/', Region, '/AUST/2011Census_G02_AUS_', Region, '.csv')) %>%
...
list.files(path = paste0('./data-raw/data-packs/data/', Region, '/AUST/'),
             pattern = paste0('2011Census_G13[A-Z]_AUS_', Region, '\\.csv$'),
             full.names = TRUE) %>%
...
if (Region == "SA1") {
  SA1_decoder <- unique(get(ced_tbls[1]), by = "SA1_CODE_2011")[[Region_key]]
  save(SA1_decoder,
       file = provide.file("~/Census2011.DataPack.SA1/data/SA1_intrnl_decoder.rda"))
}
...
if (Region == "SA1") {
  data_path <- provide_normalize("~/Census2011.DataPack.SA1/data")
  tsv_path <- provide_normalize(paste0("~/Census2011.DataPack.SA1/tsv/", Region))
} else {
  if (Region == "SSC") {
    data_path <- provide_normalize("~/Census2011.DataPack.SSC/data")
    tsv_path <- provide_normalize(paste0("~/Census2011.DataPack.SSC/tsv/", Region))
  } else {
    data_path <- "data"
...
if (Region == "STE") {
  ...
  CatR("#' @name Census2011__Data")
  CatR("#' @description Data from the 2011 Australian Census")
  CatR("#' NULL")
}
...
for (tbl in ced_tbls) {
  ...
  if (Region == "SA1") {
    ...
    .ced_tbl[, "SA1_internal_2011" := fastmatch::fmatch(SA1_CODE_2011, SA1_decoder)]
    .ced_tbl[, "SA1_CODE_2011" := NULL]
    set_cols_first(.ced_tbl, "SA1_internal_2011")
  }
